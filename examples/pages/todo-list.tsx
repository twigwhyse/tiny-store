...
class TodoStore extends ReactStore<AppState> {
  ...
  toggleTodo = (id: string) => {
    this.setState({
      todos: op.updateAt<Todo>(op.idIndex(id), op.partial({ completed: (v) => !v })),
    })
  }

  addTagToTodo = (todoId: string, tag: string) => {
    this.setState({
      todos: op.updateAt<Todo>(op.idIndex(todoId), op.partial({ tags: op.add(tag) })),
    })
  }

  removeTagFromTodo = (todoId: string, tag: string) => {
    this.setState({
      todos: op.updateAt<Todo>(op.idIndex(todoId), op.partial({ tags: op.remove(tag) })),
    })
  }
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto', padding: '20px' }}>
      ...
      <header style={{ marginBottom: '30px', textAlign: 'center' }}>
        <h1>{user.avatar} {user.name}'s Todos</h1>
        <div>
          <input 
      ...
      <div style={{ marginBottom: '20px' }}>
        <label style={{ marginRight: '20px' }}>
          <input 
            ...
            onChange={(e) => todoStore.updateFilters({ showCompleted: e.target.checked })}
          />
          显示已完成
        </label>
...
// 添加待办表单组件
function AddTodoForm({ onAdd }: { onAdd: (text: string, tags?: string[]) => void }) {
    ...
    <div style={{ 
      ...
      <div style={{ fontSize: '0.9em' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center' }}>
          ...
          />
          <button onClick={addTag} style={{ marginLeft: '5px', fontSize: '0.8em' }}>
            +
          </button>
        </div>
